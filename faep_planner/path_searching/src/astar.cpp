...
int Astar::search(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt, Eigen::Vector3d cur_vel, bool dynamic, double time_start) {
  /* ---------- initialize ---------- */
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          Eigen::Vector3d guideDir = (pro_pos - cur_pos).normalized();
          double angle = acos(guideDir.dot(cur_vel.normalized())) * 180 / 3.141593;
          if(angle > 180) angle = 360 - angle;

          if(iter_num_ <= 10)
          {
            tmp_f_score = tmp_g_score + lambda_heu_ * getEuclHeu(pro_pos, end_pt) + angle/iter_num_;
          }
          else tmp_f_score = tmp_g_score + lambda_heu_ * getEuclHeu(pro_pos, end_pt);
