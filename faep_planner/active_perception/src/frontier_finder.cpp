...
namespace fast_planner
{
  ...
  void FrontierFinder::computeFrontiersToVisit()
  {
    ...
    for (auto &tmp_ftr : tmp_frontiers_)
    {
      ...
      {
        // Find no viewpoint, move cluster to dormant list     暂时没有视点可以观测的边界
      ...
      Eigen::MatrixXd &mat)
  {
    ...
    else
    {
      ...
      for (auto &ftr : frontiers_)
      {
        ...
        //zyh
        // int surfaceVoxelNum = 0;
        // for (auto cell : ftr.cells_)
        // {
        //   Eigen::Vector3i curIndex;
        //   edt_env_->sdf_map_->posToIndex(cell, curIndex);
        //   if (isNeighborOccupied(curIndex))
        //     surfaceVoxelNum++;
        // }
